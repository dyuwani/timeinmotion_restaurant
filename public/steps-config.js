/* ═══════════════════════════════════════════════════════
   ...
   before either page's own inline script (and after theme.css/theme.js).

   Colors below are CSS custom-property references, not literal hex —
   theme.css defines the dark (default) and light values per token, so
   toggling the theme repaints everything live with no re-render needed.
═══════════════════════════════════════════════════════ */
const TYPE_COLORS = { setup:"var(--c-setup)",order:"var(--c-order)",fire:"var(--c-fire)",serve:"var(--c-serve)",clear:"var(--text-3)",payment:"var(--c-payment)",close:"var(--c-close)" };
...
const FINEDINING_GROUPS = [
  { id:"setup",label:"Table Setup",icon:"🪑",color:"var(--c-setup)",steps:[
  ...
  { id:"order",label:"Order Phase",icon:"📋",color:"var(--c-order)",steps:[
  ...
  { id:"amuse",label:"Amuse-Bouche",icon:"✨",color:"var(--c-fire)",steps:[
  ...
  { id:"bread",label:"Bread Service",icon:"🍞",color:"var(--c-bread)",steps:[
  ...
  { id:"starter",label:"Starter / First Course",icon:"🔥",color:"var(--c-fire)",steps:[
  ...
  { id:"intermezzo",label:"Intermezzo",icon:"❄️",color:"var(--c-violet)",steps:[
    {id:16,step:"Intermezzo / Sorbet",activity:"Palate Cleanser Delivered",type:"serve",optional:true},
  ]},
  { id:"main",label:"Main Course",icon:"🔥",color:"var(--c-fire)",steps:[
  ...
  { id:"cheese",label:"Cheese Course",icon:"🧀",color:"var(--c-cheese)",steps:[
  ...
  { id:"dessert",label:"Dessert",icon:"🔥",color:"var(--c-fire)",steps:[
  ...
  { id:"payment",label:"Settlement",icon:"💳",color:"var(--c-payment)",steps:[
...
const CLUBS_GROUPS = [
  { id:"arrival",label:"Arrival & Seating",icon:"🚪",color:"var(--c-setup)",steps:[
  ...
  { id:"bottle",label:"Bottle Service",icon:"🍾",color:"var(--c-fire)",steps:[
  ...
  { id:"reorder",label:"Additional Round",icon:"🔄",color:"var(--c-violet)",steps:[
...
const USE_CASES = {
  ...
  clubs:{
    ...
    chartColor:"var(--c-clubs)",
  },
};
// Fixed categorical order for venue charts — validated for CVD separation against
// both a dark and a light surface (see dataviz palette checks in theme.css's
// comments). Never reassign a venue a different slot.
const VENUE_COLORS = { "Revel":"var(--c-venue-1)", "Yes Please":"var(--c-venue-2)", "The Clubhouse":"var(--c-venue-3)" };
...
/* A 6-hex-digit color mixed to P% opacity, live-reactive to the CSS variable
   it wraps (so it repaints correctly when the theme toggles). Use in place
   of the old "hex + alpha-suffix" string-concat trick, which doesn't work
   once colors are var(--x) references instead of literal hex. */
